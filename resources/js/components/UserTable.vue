  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>{{title}} Table</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
  ...
  export default {
    ...
    methods: {
      initialize () {
        ...
        axios.get('api/user/'+ this.type, {headers: {"Authorization" : "Bearer " + token}})
        .then(reponse => {
      ...
      save () {
        if (this.editedItem.id) {
          ...
          if(this.password != "" && this.confirmPassword != ""){
            ...
            const token = window.localStorage.getItem('token');
            axios.put('api/user/' + this.type + '/' + this.editedItem.id, editedUser, {headers: {"Authorization" : "Bearer " + token}})
            .then(res  => {
              if(res.data.res === "ok"){
                alert("Success")
                this.resetError()
            ...
            const token = window.localStorage.getItem('token');
             axios.post('api/user/'+ this.type, editedUser,  {headers: {"Authorization" : "Bearer " + token}})
             .then(res  => {
                if(res.data.res === "ok"){
                  alert("Success");
                  this.resetError();
